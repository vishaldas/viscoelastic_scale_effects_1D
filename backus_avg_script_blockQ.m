...
% Blocking Qp to 20
Qp = 20.*(Qp < 250) + 250.*(Qp == 250);
Qp = 250.*(Depth2<2150) + Qp.*(Depth2>=2150);
...
%% Plots of only viscoelastic region
...
xlabel('Qp^{-1}'); 
ylabel('Depth');
title('Qp^{-1}', 'FontWeight', 'bold'); 

subplot 133
plot(vp_elast_bcks2, Depth2, '-k');
hold on;
plot(vp_visco_bcks, Depth2, '--k');
ylim([2130 2200]);
axis ij; box on;
xlabel('Vp (m/sec)'); 
ylabel('Depth');
title('Upscaled Vp', 'FontWeight', 'bold');
